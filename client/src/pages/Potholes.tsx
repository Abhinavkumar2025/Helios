...
export const Potholes: React.FC = () => {
  ...
  const loadPotholes = async () => {
    try {
      setLoading(true);
      const data = await fetchIncidents({ event_type: "pothole", limit: 60 });
      setPotholes(data);
    } catch (err) {
  ...
  const criticalCount = potholes.filter((p) => p.severity === "critical" || p.severity === "high").length;
  const moderateCount = potholes.filter((p) => p.severity === "medium").length;
              ...
              {loading && potholes.length === 0 ? (
                ...
                potholes.map((p) => (
                  ...
                  >
                    ...
                    <td className="py-3.5 font-semibold text-slate-200">{p.bus_id}</td>
          ...
          <div className="space-y-4 font-mono text-xs">
            ...
            <div className="grid grid-cols-2 gap-2 text-slate-300">
              <div className="p-3 rounded-lg bg-helios-850 border border-slate-800">
                <span className="text-slate-500 uppercase text-[10px] block">Bus Source</span>
                <span className="font-bold text-white text-sm">{selectedPothole.bus_id}</span>
              </div>
              <div className="p-3 rounded-lg bg-helios-850 border border-slate-800">
